 ...
 * `base` es parametrizable por flujo (Meal Prep vs Cocinera); por defecto Meal Prep.
 */
export function computeTotal({ base = MEAL_PREP_BASE, costo_despacho = 0, bakingTotal = 0 } = {}) {
  return Number(base || MEAL_PREP_BASE) + Number(costo_despacho || 0) + Number(bakingTotal || 0)
}
